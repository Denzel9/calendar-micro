  ...
  return (
    <div className=" z-20 fixed left-0 top-0 bottom-0 w-24 p-5 bg-black flex flex-col items-center border-r border-grey">
      ...
      <div className=" flex flex-col gap-5 mt-20">
        {menu.map((el) => <SidebarItem key={Math.random()} icon={el.icon} link={el.link} />).slice(0, 3)}
      </div>
      <div className=" border-b-2 border-grey w-10 my-10" />
      <div className=" flex flex-col gap-5">
        {menu.map((el) => <SidebarItem key={Math.random()} icon={el.icon} link={el.link} />).slice(3)}
      </div>
      <div className=" absolute bottom-10">
        <SidebarItem icon={<IoExitOutline />} onClick={() => {}} />
      </div>
    </div>
  )
}
